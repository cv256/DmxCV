    ...
    for(bus=usb_busses; bus; bus=bus->next){
        for(dev=bus->devices; dev; dev=dev->next)
        {
            if(dev->descriptor.idVendor == USBDEV_SHARED_VENDOR && dev->descriptor.idProduct == USBDEV_SHARED_PRODUCT)
            {
                ...
                fprintf(stderr, "info: SharedVendor found\n");
                ...
                fprintf(stderr, "info: got Descriptor.iManu [%s]\n", string);

                /* fprintf(stderr, "seen device from vendor ->%s<-\n", string); */
                if(strcmp(string, "www.anyma.ch") != 0)
                {
                  fprintf(stderr, "info: found device: %s\n", string);
                  goto skipDevice;
                }
                ...
                fprintf(stderr, "info: got Descriptor.iProd [%s]\n", string);
...
skipDevice:
            ...
            else
            {
              ...
              fprintf(stderr, "  info: no SharedVendor / Product: ");
              ...
              len = usbGetStringAscii(handle, dev->descriptor.iManufacturer, 0x0409, string, sizeof(string));
              if(len > 0)
                fprintf(stderr, "Manufacturer[%-40s] ", string);

              len = usbGetStringAscii(handle, dev->descriptor.iProduct, 0x0409, string, sizeof(string));
              if(len > 0)
                fprintf(stderr, "Product[%-40s]", string);
